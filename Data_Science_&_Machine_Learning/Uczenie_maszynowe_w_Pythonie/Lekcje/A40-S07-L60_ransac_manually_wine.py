...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  3 15:26:10 2021
...
class RANSAC1:
    ...
    def __init__(self, max_iters_k=100, treshold=0.5, 
                 min_acceptable_inliers=100):
    ...
    def fit(self, X, y, show_partial_results=False):
        ...
        assert X.shape[0] > 1.5 * self.min_acceptable_inliers, 'The input matrxi should'
        ...
        for i in range(self.max_iters_k):
            ...
            rand_idx = np.random.choice(len(data), size=sample_size, 
                                        replace=False)
            points = data[rand_idx,:]
            
            # determine the line equation
            # tangens nachylenia prostej
            
            # sys.float_info.epsilon - mirko liczba aby uniknąć ewentualnego 
            # dzielenia przez 0
            ...
            if show_partial_results:
                ...
                plt.scatter(X[this_inlier_mask], y[this_inlier_mask], 
                            color='green', marker='.', label='Inliers')
                plt.scatter(X[~this_inlier_mask], y[~this_inlier_mask], 
...
plt.scatter(X[:], y[:], color='green', marker='.', label='Inliers')
